...
public class JwtClientAssertionParametersService {
  ...
  public MultiValueMap<String, String> buildClientAssertionParameters(
    ...
    if (orgNumber != null) {
      String authorization_details = String.format(AUTHORIZATION_DETAILS_ORG_NR_TEMPLATE,
          orgNumber);
    ...
    JwsEncoderHolder jwsEncoderHolder = this.jwsEncoders.compute(
        clientRegistration.getRegistrationId(),
      ...
      if (clientRegistration
        ...
        try {
          ...
          if (registration.getPrivateKey().endsWith(".pem")) {
            ...
            final String pem = JWK2PEM.getPem(
                PathResolver.getInputStream(registration.getPrivateKey()));
            privateKey = (RSAPrivateKey) CertificateUtils.getPrivateKey(pem);
          } else {
